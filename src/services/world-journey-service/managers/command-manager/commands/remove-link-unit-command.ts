...
export class RemoveLinkUnitCommand extends Command {
  ...
  static create(unitId: string) {
    return new RemoveLinkUnitCommand(generateUuidV4(), DateVo.now(), false, unitId);
  }

  static createRemote(id: string, createdAt: DateVo, unitId: string) {
    return new RemoveLinkUnitCommand(id, createdAt, true, unitId);
  }
